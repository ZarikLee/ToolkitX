      ...
      <div className="flex">
        ...
        <main className="flex-1 min-w-0 px-6 py-6">
          ...
          <div className="rounded-lg border p-4 mb-6" style={{ background: "var(--surface-container-low)", borderColor: "var(--outline-variant)" }}>
            <p className="text-sm" style={{ color: "var(--on-surface-variant)" }}>从左侧选择教程开始学习，或点击下方链接进入第一篇。</p>
            <Link href={`/learn/${categoryId}/${category.tutorials[0]?.slug}`} className="inline-block mt-3 px-4 py-2 rounded text-sm font-medium" style={{ background: "var(--secondary)", color: "var(--on-secondary)" }}>
              开始学习 →
            </Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {category.tutorials.map((tutorial, i) => (
              <Link key={tutorial.slug} href={`/learn/${categoryId}/${tutorial.slug}`} className="flex items-center gap-3 p-3 rounded-lg border transition-colors hover:opacity-80" style={{ borderColor: "var(--outline-variant)", background: "var(--surface-container-lowest)" }}>
